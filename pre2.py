...
# 读取融合后的 CSV 文件
df = pd.read_csv('E:\\camera\\data\\df_merged.csv')

# 针对 test 数据，假设经纬高分别存放在 'Xt', 'Yt', 'Zt'
def compute_test_ecef(row):
    if pd.isna(row['lon_t']) or pd.isna(row['lat_t']) or pd.isna(row['Ht']):
        # 指定返回 Series 的索引，保证长度一致
        return pd.Series([None, None, None], index=['Xt_ecef', 'Yt_ecef', 'Zt_ecef'])
    return pd.Series(geodetic_to_ecef(row['lon_t'], row['lat_t'], row['Ht']),
                     index=['Xt_ecef', 'Yt_ecef', 'Zt_ecef'])

# 新增 test 数据对应的 ECEF 坐标列
df[['Xt_ecef', 'Yt_ecef', 'Zt_ecef']] = df.apply(compute_test_ecef, axis=1)
...
# 保存转换后的结果到新的 CSV 文件
# df.to_csv('E:\\camera\\df_merged_with_ecef.csv', index=False)
